...
ll solve(ll idx,ll state)
{
    ...
    ll mx=LLONG_MIN;
    mx=max(mx,solve(idx+1,state)) ;  ///nimu na
   //ll mx=solve(idx+1,0);

    if(state) mx=max(mx,-a[idx]+solve(idx+1,0));
    else mx=max(mx,a[idx]+solve(idx+1,1));

  //  cout<<"IDX "<<idx<<" MX "<<mx<<endl;

    return dp[idx][state]=mx;
